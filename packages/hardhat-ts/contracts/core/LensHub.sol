...
contract LensHub is LensNFTBase, VersionedInitializable, LensMultiState, LensHubStorage, ILensHub {
  ...
  function setDefaultProfileWithSig(DataTypes.SetDefaultProfileWithSigData calldata vars) external override whenNotPaused {
    unchecked {
      _validateRecoveredAddress(
        _calculateDigest(
          keccak256(abi.encode(SET_DEFAULT_PROFILE_WITH_SIG_TYPEHASH, vars.wallet, vars.profileId, sigNonces[vars.wallet]++, vars.sig.deadline))
        ),
        vars.wallet,
        vars.sig
      );
  ...
  function setFollowModule(
    ...
    PublishingLogic.setFollowModule(profileId, followModule, followModuleInitData, _profileById[profileId], _followModuleWhitelisted);
  }
  ...
  function setFollowModuleWithSig(DataTypes.SetFollowModuleWithSigData calldata vars) external override whenNotPaused {
    ...
    PublishingLogic.setFollowModule(vars.profileId, vars.followModule, vars.followModuleInitData, _profileById[vars.profileId], _followModuleWhitelisted);
  }
  ...
  function setFollowNFTURIWithSig(DataTypes.SetFollowNFTURIWithSigData calldata vars) external override whenNotPaused {
    ...
    unchecked {
      _validateRecoveredAddress(
        _calculateDigest(
          keccak256(
            abi.encode(SET_FOLLOW_NFT_URI_WITH_SIG_TYPEHASH, vars.profileId, keccak256(bytes(vars.followNFTURI)), sigNonces[owner]++, vars.sig.deadline)
          )
        ),
        owner,
        vars.sig
  ...
  function post(DataTypes.PostData calldata vars) external override whenPublishingEnabled returns (uint256) {
    ...
    return _createPost(vars.profileId, vars.contentURI, vars.collectModule, vars.collectModuleInitData, vars.referenceModule, vars.referenceModuleInitData);
  }

  /// @inheritdoc ILensHub
  function postWithSig(DataTypes.PostWithSigData calldata vars) external override whenPublishingEnabled returns (uint256) {
    address owner = ownerOf(vars.profileId);
    unchecked {
      _validateRecoveredAddress(
        _calculateDigest(
  ...
  /// @inheritdoc ILensHub
  function commentWithSig(DataTypes.CommentWithSigData calldata vars) external override whenPublishingEnabled returns (uint256) {
    address owner = ownerOf(vars.profileId);
    unchecked {
      _validateRecoveredAddress(
        _calculateDigest(
  ...
  /// @inheritdoc ILensHub
  function mirrorWithSig(DataTypes.MirrorWithSigData calldata vars) external override whenPublishingEnabled returns (uint256) {
    address owner = ownerOf(vars.profileId);
    unchecked {
      _validateRecoveredAddress(
        _calculateDigest(
  ...
  /// @inheritdoc ILensHub
  function follow(uint256[] calldata profileIds, bytes[] calldata datas) external override whenNotPaused returns (uint256[] memory) {
    return InteractionLogic.follow(msg.sender, profileIds, datas, FOLLOW_NFT_IMPL, _profileById, _profileIdByHandleHash);
  }

  /// @inheritdoc ILensHub
  function followWithSig(DataTypes.FollowWithSigData calldata vars) external override whenNotPaused returns (uint256[] memory) {
    ...
    return InteractionLogic.follow(vars.follower, vars.profileIds, vars.datas, FOLLOW_NFT_IMPL, _profileById, _profileIdByHandleHash);
  }
  ...
  function collect(
    ...
    return InteractionLogic.collect(msg.sender, profileId, pubId, data, COLLECT_NFT_IMPL, _pubByIdByProfile, _profileById);
  }

  /// @inheritdoc ILensHub
  function collectWithSig(DataTypes.CollectWithSigData calldata vars) external override whenNotPaused returns (uint256) {
    unchecked {
      _validateRecoveredAddress(
        _calculateDigest(
    ...
    return InteractionLogic.collect(vars.collector, vars.profileId, vars.pubId, vars.data, COLLECT_NFT_IMPL, _pubByIdByProfile, _profileById);
  }
  ...
  function _createComment(DataTypes.CommentData memory vars) internal returns (uint256) {
    unchecked {
      ...
      PublishingLogic.createComment(vars, pubId, _profileById, _pubByIdByProfile, _collectModuleWhitelisted, _referenceModuleWhitelisted);
      return pubId;
    }
  }
