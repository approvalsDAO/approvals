...
contract LensPeriphery {
  ...
  function setProfileMetadataURIWithSig(DataTypes.SetProfileMetadataWithSigData calldata vars) external {
    _validateRecoveredAddress(
      _calculateDigest(
        keccak256(
          abi.encode(SET_PROFILE_METADATA_WITH_SIG_TYPEHASH, vars.profileId, keccak256(bytes(vars.metadata)), sigNonces[vars.user]++, vars.sig.deadline)
        )
      ),
      vars.user,
      vars.sig
   ...
   * @param profileId The profile ID to query the metadata URI for.
   */
  function getProfileMetadataURIByOwner(address user, uint256 profileId) external view returns (string memory) {
